...
- **`capture`** — finds the Claude session in the focused cmux workspace, scrapes the draft from the terminal screen (`cmux read-screen`), clears the box, appends the draft to a per-surface queue file. Idle session: just presses Enter instead. A marker file — stamped by the daemon at the keypress itself, before the capture process even starts — makes the statusline show a transient `⏳ Queue: …` placeholder while the capture is still working, so visible feedback is about a second away even before the box clears.
